...
//   std::cout << "T_START: " << t0 << std::endl;
//   std::cout << "preplanned frames " << TP.A.prePlannedFrames.N << std::endl;
//   for (auto a: TP.A.prePlannedFrames){
//     std::cout << a << std::endl;
//   }
...
//           j["goal_conf"] = q1;
//           j["start_time"] = t0;
//           j["travel_time"] = arrival_time;
//           j["path_length"] = path_length;
          
  
//           j["total_rrt_time"] = rrt_duration;
//           j["total_coll_time"] = planner.edge_checking_time_us;
//           j["total_nn_time"] = planner.nn_time_us;
//           j["dimension"] = res.path.d1;
...
//           std::cout << "Путь робота записан в файл robot_path.json" << std::endl;
//         }
...
//         // TP.init_safe_interval_collisison_check(q0);
//         const auto rrt_start_time = std::chrono::high_resolution_clock::now();
...
//         // std::cout << "time: " << res.time.N << " path d0: " << res.path.d0 << std::endl;
//         // for (int ii=0; ii < res.time.N; ii++) {
//         //     json path_point;
//         //     path_point["configuration"] = res.path[ii]; // Вектор конфигурации
//         //     path_point["time"] = res.time(ii);         // Время
//         //     j["path"].push_back(path_point);          // Добавляем точку в путь
//         // }
...
class PrioritizedTaskPlanner {
  public:
    ...
    PlanStatus plan(rai::Configuration &C, const RobotTaskPair &rtp,
      ...
      const auto pairs = get_cant_collide_pairs(TP.C);
      // std::cout<<"deactivated_pairs:"<<std::endl;
      // for (int i=0;i<pairs.d0;i++){
      // std::cout<<TP.C.getFrames(TUP(pairs(i,0)))(0)->name<<" "<<TP.C.getFrames(TUP(pairs(i,1)))(0)->name<<std::endl;
